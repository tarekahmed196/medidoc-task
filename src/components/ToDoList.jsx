...
const ToDoList = ({dataFromChild,isDesktop, isTablet, isMobile}) => {
    ...
    return (
        <section className={` mt-6 px-6 py-4 ${isDesktop  ?  ' full-body ': ''} ${isTablet ? 'full-body2' :''} ${isMobile ? 'full-body2 bg-white' : ""} ${dataFromChild? ' bg-white ': 'bg-black '} `}>
            <h2 className={`text-base py-2 ${dataFromChild?'text-black':'text-white'}`}>To-Do List</h2>
            <div className='flex gap-4 items-start'>
                
                <div className={`text-area ${dataFromChild?'':'bg-[#959190]'} ${isTablet ? 'relative' : ''} ${isMobile ? 'relative' : ''}`}>
                {
            ...
            <div className='flex flex-col gap-4'>
                <div className='flex gap-4 items-center'>
                    ...
                    <div className={`flex ${isTablet? 'flex-col': ''} ${isMobile? 'flex-col': ''}`}>
                        <span className={`text-sm ${dataFromChild?'text-black':'text-white'}`}>Lorem ipsum dolor sit amet consectetur adipisicing elit.</span>
